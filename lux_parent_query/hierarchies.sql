...
AS (SELECT ao.id as id
    ...
    #AND JSON_UNQUOTE(JSON_EXTRACT(resource.identifier, '$[0]')) in ('MS 326', 'MS 573', 'MS 1111')
    UNION ALL
    SELECT ao.id as id
		, ao.display_string as display_string
        , ao.parent_id as parent_id
	...
	JOIN resource on resource.id  = ao.root_record_id
    LEFT JOIN enumeration_value ev on ev.id = ao.level_id)
    #WHERE JSON_UNQUOTE(JSON_EXTRACT(resource.identifier, '$[0]')) in ('MS 326', 'MS 573', 'MS 1111'))
	SELECT id
		, parent_id
		, series_id
        , root_record_id
